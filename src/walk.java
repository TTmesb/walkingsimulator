...
public class walk extends Canvas implements Runnable {
    ...
    public walk() { //fönstret
        ...
        isFinished = false;
        try {
            gubbeimg = ImageIO.read(new File("RUN_003.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        gubbe = new Rectangle(200,0,80,80);
    }

    //sprittar
    private BufferedImage gubbeimg;
    private int gubbex, gubbey,gubbed,gubbew;
    private Rectangle gubbe;
    private int pelarex,pelarey;
    private int dödarex,dödarey;
    ...
    public void rita() { // vad som ska ritas
        ...
        golv(g,0,0);
        g.drawImage(gubbeimg,gubbex,gubbey,80,80,null);
        pelare(g, 200,200);
        dödare(g, 200,200);
    ...
    private void dödare(Graphics g, int x, int y){
        g.setColor(new Color(0xFFBE00));
        g.fillRect(dödarex,dödarey,10,80);
    }
    private void gubawdawdbe(Graphics g, int x, int y){
